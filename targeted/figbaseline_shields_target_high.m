...
% figure is drawn
tmpfilename = 'figbaseline_shields_target_high';
...
pars.Rshields_deploy = (load('ga_shields_delpoyment_alpha2_highCase.txt'))';
...
[t,~,te,ye,ie]=ode45(@stir_model_youngshields_target,[0:1:outbreak.pTime], outbreak.y0,opts,pars,agepars);
...
% Stats
stats_optimized.R=y(:,agepars.R_ids);
stats_optimized.D=y(:,agepars.D_ids);
stats_optimized.Htot=y(:,agepars.Ihsub_ids)+y(:,agepars.Ihcri_ids);
stats_optimized.Hacu=y(:,agepars.Ihcri_ids);
stats_optimized.Dday_age=stats_optimized.D(2:end,:)-stats_optimized.D(1:end-1,:);
stats_optimized.Dday=sum(stats_optimized.Dday_age');
stats_optimized.Hacu_day=sum(stats_optimized.Hacu');
stats_optimized.lock=y(:,agepars.Slock_ids);

% Sims - Intervene High
pars.alpha=20;  % Shielding
pars.Rshields_deploy = (load('ga_shields_delpoyment_alpha20_highCase.txt'))';
[th,yh]=ode45(@stir_model_youngshields_target,[0:1:outbreak.pTime], ye,opts,pars,agepars);

% Stats
statsh_optimized.R=yh(:,agepars.R_ids);
statsh_optimized.D=yh(:,agepars.D_ids);
statsh_optimized.Htot=yh(:,agepars.Ihsub_ids)+yh(:,agepars.Ihcri_ids);
statsh_optimized.Hacu=yh(:,agepars.Ihcri_ids);
statsh_optimized.Dday_age=statsh_optimized.D(2:end,:)-statsh_optimized.D(1:end-1,:);
statsh_optimized.Dday=sum(statsh_optimized.Dday_age');
statsh_optimized.Hacu_day=sum(statsh_optimized.Hacu');
statsh_optimized.lock=yh(:,agepars.Slock_ids);
...
% Stats
stats_uniform.R=y(:,agepars.R_ids);
stats_uniform.D=y(:,agepars.D_ids);
stats_uniform.Htot=y(:,agepars.Ihsub_ids)+y(:,agepars.Ihcri_ids);
stats_uniform.Hacu=y(:,agepars.Ihcri_ids);
stats_uniform.Dday_age=stats_uniform.D(2:end,:)-stats_uniform.D(1:end-1,:);
stats_uniform.Dday=sum(stats_uniform.Dday_age');
stats_uniform.Hacu_day=sum(stats_uniform.Hacu');
stats_uniform.lock=y(:,agepars.Slock_ids);
...
% Stats
statsh_uniform.R=yh(:,agepars.R_ids);
statsh_uniform.D=yh(:,agepars.D_ids);
statsh_uniform.Htot=yh(:,agepars.Ihsub_ids)+yh(:,agepars.Ihcri_ids);
statsh_uniform.Hacu=yh(:,agepars.Ihcri_ids);
statsh_uniform.Dday_age=statsh_uniform.D(2:end,:)-statsh_uniform.D(1:end-1,:);
statsh_uniform.Dday=sum(statsh_uniform.Dday_age');
statsh_uniform.Hacu_day=sum(statsh_uniform.Hacu');
statsh_uniform.lock=yh(:,agepars.Slock_ids);

% Sims - Baseline
% Irrelevant, no shielding
pars.alpha=0;
[tb,yb]=ode45(@stir_model_youngshields_target,[0:1:outbreak.pTime], ye,opts,pars,agepars);
% Stats
...
hold on
tmph=plot(t(2:end),stats_uniform.Dday*100000,'k--');
set(tmph,'linewidth',3);
tmph=plot(t(2:end),statsh_uniform.Dday*100000,'k:');
set(tmph,'linewidth',3);
tmph=plot(t(2:end),stats_optimized.Dday*100000,'b--');
set(tmph,'linewidth',3);
tmph=plot(t(2:end),statsh_optimized.Dday*100000,'b:');
set(tmph,'linewidth',3);
set(gca,'fontsize',16);
...
ylim([0 15]);
legend('Baseline','2:1 Shielding Uniform','20:1 Shielding Uniform', '2:1 Shielding Optimized', '20:1 Shielding Optimized');
legend('boxoff');
xlim([0 365]);


subplot(3,1,2);
tmph=plot(t,statsb.Hacu_day*100000,'k-');
set(tmph,'linewidth',3);
hold on
tmph=plot(t,stats_uniform.Hacu_day*100000,'k--');
set(tmph,'linewidth',3);
tmph=plot(t,statsh_uniform.Hacu_day*100000,'k:');
set(tmph,'linewidth',3);
tmph=plot(t,stats_optimized.Hacu_day*100000,'b--');
set(tmph,'linewidth',3);
tmph=plot(t,statsh_optimized.Hacu_day*100000,'b:');
set(tmph,'linewidth',3);
set(gca,'fontsize',16);
xlabel('Time, days','fontsize',16,'verticalalignment','top','interpreter','latex');
...
set(gca,'fontsize',16);
hold on
tmpp=plot(agepars.meanage,stats_uniform.D(end,:)*100000,'ko--');
set(tmpp,'linewidth',3);
tmpp=plot(agepars.meanage,statsh_uniform.D(end,:)*100000,'ko:');
set(tmpp,'linewidth',3);
tmpp=plot(agepars.meanage,stats_optimized.D(end,:)*100000,'bo--');
set(tmpp,'linewidth',3);
tmpp=plot(agepars.meanage,statsh_optimized.D(end,:)*100000,'bo:');
set(tmpp,'linewidth',3);
...
tmpl = legend('Baseline','2:1 Shielding Uniform','20:1 Shielding Uniform', '2:1 Shielding Optimized', '20:1 Shielding Optimized', 'Age Structure' );
