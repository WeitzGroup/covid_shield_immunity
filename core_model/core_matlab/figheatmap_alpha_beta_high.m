...
% figure is drawn
tmpfilename = 'figheatmap_alpha_beta_high';
...
fid =fopen('alpha_beta_intervene_high.mat');
if (fid <0)
  ...
  for i=1:length(pars.sdistance),
    for j=1:length(pars.alpha_range),
      i
      pars.beta_a=3/10*(1-pars.sdistance(i));   % Transmission for asymptomatic
      pars.beta_s=6/10*(1-pars.sdistance(i));      % Transmission for symptomatic
  ...
  save alpha_beta_intervene_high pars agepars population outbreak stats tb yb tpre ypre
else
  load alpha_beta_intervene_high
end

imagesc(pars.alpha_range,pars.sdistance,stats.dead_reduce)
axis square
set(gca,'YDir','normal');       
hold on
[c,h]=contour(pars.alpha_range,pars.sdistance,stats.dead_reduce,[0.2 0.4 0.5 0.6 0.7 0.8 0.9 0.95 0.99]);
set(h(1),'linewidth',2,'color','white')
ylim([0.1 0.4]);
colormap('default');
set(gca,'yticklabels',{'10%','15%','20%','25%','30%','35%','40%'});
cmap=colormap;
cmap=cmap(end:-1:1,:);
colormap(cmap);
tmpc=colorbar;
...
xlabel('Shielding, $\alpha$','fontsize',20,'verticalalignment','top','interpreter','latex');
ylabel({'Social distancing';'percent reduction in transmission'},'fontsize',20,'verticalalignment','bottom','interpreter','latex');
title({'Relative reduction in total deaths';'High Scenario, ${\cal{R}}_0=2.33$'},'fontsize',20,'interpreter','latex')
set(gca,'fontsize',20);
