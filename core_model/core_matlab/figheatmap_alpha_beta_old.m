...
% figure is drawn
tmpfilename = 'figheatmap_alpha_beta_old';
...
pars.shield_old = (1-pars.p).*population.agefrac.*agepars.hosp_frac.*agepars.hosp_crit.*agepars.crit_die;
pars.shield_old=pars.shield_old/sum(pars.shield_old);
...
% 4.5 Day doubling time in exponential phase - close to reality
%semilogy(t',(1-sum(y(:,agepars.S_ids)')));
%[p,s]=polyfit(t',log(1-sum(y(:,agepars.S_ids)')),1)
...
fid =fopen('alpha_beta_intervene.mat');
if (fid <0)
  ...
  save alpha_beta_intervene pars agepars population outbreak stats tb yb tpre ypre
else
  load alpha_beta_intervene
end
...
set(gca,'YDir','normal');       
hold on
[c,h]=contour(pars.alpha_range,pars.sdistance,stats.dead_reduce);
...
title('Relative reduction in total deaths, 1 year horizon','fontsize',18,'interpreter','latex')
