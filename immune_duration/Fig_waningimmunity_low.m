...
% The simulations below correspond to the low R0 scenario
% FigS8-right
...
for i = 1:length(mean_immuneduration) % Find the starting point for intervention, i.e., when cases reach 10,000
    % Shield parameters
...
save ye_matlow.mat ye_mat
load ye_matlow.mat

% We test different mean immunity durations ranging from 2 to 12 months
...
save stats_alpha0low.mat stats_alpha0
save stats_alpha2low.mat stats_alpha2
save stats_alpha20low.mat stats_alpha20


%% Plot the results (high R0 scenario)
% cumulative deaths, Max ICU beds and total cases
load stats_alpha2low.mat
load stats_alpha20low.mat
load stats_alpha0low.mat
...
title({'COVID-19 Epidemic - Low Scenario - Shields Ages 20-60';'${\cal{R}}_0=1.57$'},'fontsize',18,'interpreter','latex')
xlim([2 mean_immuneduration(end)/30]);
ylim([0 max(deaths_alpha0)+10])
lg =legend('$\alpha = 0$', '$\alpha = 2$','$\alpha = 20$', 'interpreter','latex', 'Position', [0.7388    0.7583    0.1533    0.0743]);
legend('boxoff');
...
lg = legend('$\alpha = 0$', '$\alpha = 2$','$\alpha = 20$', 'interpreter','latex', 'Position', [ 0.7443    0.4530    0.1533    0.0728]);
legend('boxoff');
...
xlim([2 mean_immuneduration(end)/30]);
ylim([1e4 max(cases_alpha0*1e5)+1000]);
lg = legend('$\alpha = 0$', '$\alpha = 2$','$\alpha = 20$', 'interpreter','latex', 'position', [ 0.7516    0.1326    0.1533    0.0728]);
legend('boxoff');
set(gcf, 'position', [336    41   547   781]);
saveas(gcf, './Fig_waningimmunity_low', 'fig')
saveas(gcf, './Fig_waningimmunity_low','epsc')