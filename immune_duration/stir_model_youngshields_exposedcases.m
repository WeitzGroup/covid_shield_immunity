function dydt = stir_model_youngshields_exposedcases(t,y,pars,agepars)
% function dydt = stir_model_youngshields_exposedcases(t,y,pars,agepars)
% SEIaISR (open) and then I_ha I_hs D RH1 RH2 - 11 categories in total, all age-stratified
% For this model we also incorporate a mock equation (last eqn. in the model) to help to keep track
% of the total infected fraction over time.
...
dydt(agepars.cases_ids)= pars.beta_a*y(agepars.S_ids)*Ia/(Ntot+pars.alpha*Rshields)+pars.beta_s*y(agepars.S_ids)*Is/(Ntot+pars.alpha*Rshields);
